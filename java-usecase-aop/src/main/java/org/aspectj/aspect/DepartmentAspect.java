...
public class DepartmentAspect {
    ...
    //匹配方法执行连接点，即匹配哪个方法执行
    @Pointcut("execution(public org.aspectj.domain.Employee org.aspectj.department.DepartmentService.getEmployee(Integer))")
    ...
    //也就是说该切点会匹配类中所有方法的调用。
    @Pointcut("execution(* org.aspectj.department.DepartmentService.*(..))")
    ...
    //匹配一个类中所有方法的调用
    @Pointcut("within(org.aspectj.department.DepartmentService)")
    public void anyMethodWithinClassCut() {
    }

    //匹配某个包下面的所有类的所有方法调用
    @Pointcut("within(org.aspectj.department.*)")
